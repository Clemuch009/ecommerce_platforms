...
class BaseRedis:
    ...
    async def publish(self, channel: str, message: Any):
        client = self._get_client()
        try:
            return await client.publish(self._key(channel), message)
        except RedisError as e:
            raise RedisError(f"Publish failed to {channel}") from e

    async def subscribe(self, *channels: str):
        client = self._get_client()
        pubsub = client.pubsub(ignore_subscribe_messages=True)
        await pubsub.subscribe(*[self.key(ch) for ch in channels])
        try:
            async for message in pubsub.listen():
                if messagee['type'] == 'message':
                    yield {'channel': message['channel'].decode(), 'data': message['data'].decode()}

        except RedisError as e:
            raise RedisError(f"Subscribe failed for channels {channels}") from e
        finally:
            await pubsub.close()
